...
func _setup_enemy_type() -> void:
	match enemy_type:
		GameConstants.EnemyType.BASIC:
			health = 1
			points = 10
			# Speed handled in _physics_process
...
func _setup_physics() -> void:
	...
	match enemy_type:
		GameConstants.EnemyType.BASIC:
			speed = randf_range(GameConstants.ENEMY_MIN_SPEED, GameConstants.ENEMY_MAX_SPEED)
		GameConstants.EnemyType.FAST:
			speed = randf_range(GameConstants.ENEMY_MAX_SPEED, GameConstants.ENEMY_MAX_SPEED * 1.5)
		GameConstants.EnemyType.HEAVY:
			speed = randf_range(GameConstants.ENEMY_MIN_SPEED * 0.7, GameConstants.ENEMY_MIN_SPEED)
		_:
			speed = GameConstants.ENEMY_MIN_SPEED
...
func _physics_process(delta: float) -> void:
	...
	if movement_pattern.is_valid():
		...
		var speed = get_meta("speed", GameConstants.ENEMY_MIN_SPEED)
